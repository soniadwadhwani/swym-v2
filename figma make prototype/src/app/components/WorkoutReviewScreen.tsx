  ...
  return (
    <div className="min-h-screen bg-[#F3F1EE] pb-28">
      ...
      <div className="px-5 -mt-3 mb-5">
        <div className="grid grid-cols-4 gap-2">
          <div className="bg-white rounded-[20px] p-4 shadow-sm shadow-black/3 text-center">
            <div className="text-xl text-[#140C32] tracking-tight" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
              {workout.totalDistance >= 1000
                ? `${(workout.totalDistance / 1000).toFixed(1)}`
                : workout.totalDistance}
            </div>
            <div className="text-[8px] text-[#140C32]/20 tracking-wide mt-0.5">
          ...
          <div className="bg-white rounded-[20px] p-4 shadow-sm shadow-black/3 text-center">
            ...
            <div className="text-[8px] text-[#140C32]/20 tracking-wide mt-0.5">min</div>
          </div>
          <div className="bg-white rounded-[20px] p-4 shadow-sm shadow-black/3 text-center">
            <div className="text-xl text-[#140C32] tracking-tight" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
              {Math.floor(workout.avgPace100m / 60)}:{(workout.avgPace100m % 60).toString().padStart(2, '0')}
            </div>
            <div className="text-[8px] text-[#140C32]/20 tracking-wide mt-0.5">/100m</div>
          </div>
          <div className="bg-white rounded-[20px] p-4 shadow-sm shadow-black/3 text-center">
            <div className="text-xl text-[#140C32] tracking-tight" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
              {workout.sets.length}
            </div>
            <div className="text-[8px] text-[#140C32]/20 tracking-wide mt-0.5">sets</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 space-y-4">
        ...
        {/* Sets Breakdown */}
        <div>
          <div className="text-[#140C32]/25 text-[10px] tracking-[0.15em] mb-3 px-1">SET BREAKDOWN</div>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    <div className="w-11 h-11 rounded-2xl bg-[#707CFF]/8 flex items-center justify-center shrink-0">
                      <Icon size={20} strokeWidth={1.5} className="text-[#707CFF]" />
                    </div>
                    <div className="flex-1 text-left">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] text-[#140C32]/15">•</span>
                        <span className="text-[10px] text-[#140C32]/30">{set.stroke}</span>
                      </div>
                      <div className="text-xs text-[#140C32]/40">
                        {set.actualDistance}m · {set.laps.length} laps
                      </div>
                    </div>
                    <ChevronRight size={16} className="text-[#140C32]/15 shrink-0" />
                  </div>

                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-[#F3F1EE] rounded-xl p-3 text-center">
                      <div className="text-base text-[#140C32]" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
                        {Math.floor(set.totalTimeSeconds / 60)}:{(set.totalTimeSeconds % 60).toString().padStart(2, '0')}
                      </div>
                      <div className="text-[8px] text-[#140C32]/20 tracking-wide mt-0.5">duration</div>
                    </div>
                    <div className="bg-[#F3F1EE] rounded-xl p-3 text-center">
                      <div className="text-base text-[#140C32]" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
                        {Math.floor(set.avgPace100m / 60)}:{(set.avgPace100m % 60).toString().padStart(2, '0')}
                      </div>
                      <div className="text-[8px] text-[#140C32]/20 tracking-wide mt-0.5">avg pace</div>
                    </div>
                    <div className="bg-[#F3F1EE] rounded-xl p-3 text-center">
        ...
        <div className="bg-white rounded-[24px] p-6 shadow-sm shadow-black/3">
          <div className="flex items-center gap-2 mb-4">
            <Zap size={14} strokeWidth={1.5} className="text-[#707CFF]" />
            <span className="text-[10px] text-[#140C32]/25 tracking-[0.15em]">KEY INSIGHTS</span>
          </div>
          <div className="space-y-3">
            {workout.aiInsights.map((insight, idx) => (
              <div key={`workout-insight-${idx}`} className="flex gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-[#707CFF] mt-1.5 shrink-0" />
                <p className="text-xs text-[#140C32]/60 leading-relaxed flex-1">
                  {insight}
                </p>
              </div>
            ))}
