      ...
      <div className="px-5 -mt-3 mb-5">
        <div className="grid grid-cols-4 gap-2">
          <div className="bg-white rounded-[20px] p-3 shadow-sm shadow-black/3 text-center">
            <div className="text-lg text-[#061922] tracking-tight" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
              {Math.floor(set.totalTimeSeconds / 60)}:{(set.totalTimeSeconds % 60).toString().padStart(2, '0')}
            </div>
            <div className="text-[8px] text-[#061922]/20 tracking-wide mt-0.5">total</div>
          </div>
          <div className="bg-white rounded-[20px] p-3 shadow-sm shadow-black/3 text-center">
            <div className="text-lg text-[#061922] tracking-tight" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
              {Math.floor(set.avgPace100m / 60)}:{(set.avgPace100m % 60).toString().padStart(2, '0')}
            </div>
            <div className="text-[8px] text-[#061922]/20 tracking-wide mt-0.5">avg</div>
          </div>
          <div className={`bg-white rounded-[20px] p-3 shadow-sm shadow-black/3 text-center ${
            ...
            <div className={`text-lg tracking-tight flex items-center justify-center gap-0.5 ${
              targetDiff < 0 ? 'text-green-600' : targetDiff > 0 ? 'text-red-500' : 'text-[#061922]'
          ...
          <div className="bg-white rounded-[20px] p-3 shadow-sm shadow-black/3 text-center">
            ...
            <div className="text-[8px] text-[#061922]/20 tracking-wide mt-0.5">consist.</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 space-y-4">
        ...
        <div className="bg-white rounded-[24px] p-5 shadow-sm shadow-black/3">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="text-[#061922]/25 text-[10px] tracking-[0.15em]">LAP BY LAP PACE</div>
              <div className="text-sm text-[#061922] mt-0.5" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
                Seconds per 100m
              </div>
            </div>
          </div>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#CBC6B3" />
              <XAxis
                dataKey="lap"
                stroke="#061922"
                opacity={0.2}
              ...
              <Tooltip
                ...
                labelStyle={{ color: '#61949B', fontSize: '11px', marginBottom: '4px', fontWeight: '400' }}
              ...
              <ReferenceLine
                ...
                strokeDasharray="5 5"
                opacity={0.3}
                label={{ value: 'Target', position: 'right', fill: '#61949B', fontSize: 9 }}
              />
              <Line
                type="monotone"
                dataKey="pace"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-[24px] p-5 shadow-sm shadow-black/3">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] text-[#061922]/25 tracking-[0.15em]">FASTEST LAP</span>
            </div>
          ...
          <div className="bg-white rounded-[24px] p-5 shadow-sm shadow-black/3">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] text-[#061922]/25 tracking-[0.15em]">SLOWEST LAP</span>
            </div>
        ...
        {/* Split Analysis */}
        <div className="bg-white rounded-[24px] p-5 shadow-sm shadow-black/3">
          <div className="text-[#061922]/25 text-[10px] tracking-[0.15em] mb-4">SPLIT ANALYSIS</div>
          <div className="space-y-3">
            <div className="flex items-center justify-between py-2">
              <span className="text-xs text-[#061922]/50">First Half Avg</span>
            ...
            <div className="flex items-center justify-between py-2 border-t border-[#CBC6B3]">
              <span className="text-xs text-[#061922]/50">Second Half Avg</span>
              <span className="text-sm text-[#061922]" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
                {Math.floor(secondHalfAvg / 60)}:{(Math.round(secondHalfAvg) % 60).toString().padStart(2, '0')}/100m
              </span>
            </div>
            <div className="flex items-center justify-between py-2 border-t border-[#CBC6B3]">
              <span className="text-xs text-[#061922]/50">Pace Differential</span>
              <div className={`flex items-center gap-1 text-sm ${
                paceFade < 0 ? 'text-green-600' : paceFade > 0 ? 'text-red-500' : 'text-[#061922]'
              }`} style={{ fontFamily: 'Comfortaa, sans-serif' }}>
                {paceFade < 0 ? (
            ...
            <div className="flex items-center justify-between py-2 border-t border-[#CBC6B3]">
              <span className="text-xs text-[#061922]/50">Avg Rest Between Laps</span>
              <span className="text-sm text-[#061922]" style={{ fontFamily: 'Comfortaa, sans-serif' }}>
                {avgRest.toFixed(1)}s
              </span>
            </div>
        ...
        {/* Lap Breakdown Table */}
        <div className="bg-white rounded-[24px] p-5 shadow-sm shadow-black/3">
          <div className="text-[#061922]/25 text-[10px] tracking-[0.15em] mb-4">ALL LAPS</div>
          <div className="space-y-1">
            {set.laps.map((lap) => {
              ...
              return (
                ...
                >
                  <span className="text-[10px] text-[#061922]/20 w-8">#{lap.lapNumber}</span>
                  <span className="text-xs text-[#061922] tabular-nums flex-1">
                    {lap.timeSeconds}s
                  </span>
                  <span className="text-xs text-[#061922]/40 tabular-nums w-16 text-right">
                    {Math.floor(lap.pace100m / 60)}:{(lap.pace100m % 60).toString().padStart(2, '0')}
                  </span>
                  <span className="text-[10px] text-[#061922]/20 w-12 text-right">
                    +{lap.restAfter}s
                  </span>
                  {isFastest && (
